...
Curved and compound Canvas clips use antialiased masks, rasterized from the transformed path with the existing NanoSVG rasterizer and sampled by every NanoVG draw type. Masks are bounded to the path/canvas intersection, intersect with saved parent masks, and remain alive through deferred GPU submission. `beginPath`, immediate rectangle/image operations, and GPU flushes must not erase a saved clip; `restore` must recover its parent. Apply clipping after filters, not before blur. Regression coverage includes rounded-button text, fills, strokes, images, clears, both blur kernels, nested rounded clips, `Path2D`, winding rules, empty regions, and current-path preservation. Do not approximate a curved clip with its bounding box or by reusing it as the next fill path.
...
Legacy NPE graphs without serialized timing fields must retain the inherited particle-system defaults: `updateSpeed = 0.01` and `preWarmStepOffset = 1`. Newly authored SystemBlocks keep their current `0.0167` update speed, and explicit serialized values, including zero, remain authoritative. A zero prewarm multiplier advances no simulation time, even after hundreds of cycles. Verify particle emission and image content as well as the whole-image error: scene 103's entirely blank output differed by only 0.272083%, below its former 0.35% allowance. Its repaired first-frame capture measures 0.040417% against the unchanged canonical reference; the tightened 0.05% gate rejects the missing effect.
